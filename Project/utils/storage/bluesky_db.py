...
class SQLiteBlueSkySaver:
    ...
    def get_all_posts(self) -> List[BSPost]:
        self.cursor.execute("SELECT * FROM Posts LEFT JOIN CommentAnalysis ON Posts.id = CommentAnalysis.id WHERE CommentAnalysis.gemini_label IS NULL")
        post_rows = self.cursor.fetchall()

        self.cursor.execute("SELECT * FROM Accounts")
        accounts = {
            row[0]: BSAccount(id=row[0], username=row[1], display_name=row[2], url=row[3])
